...
suite("PruebaClaveAreaPreguntaDAO Unit Test", function () {
    var cut;
    var stubFetch;

    setup(function () {
        cut = new PruebaClaveAreaPreguntaDAO("clave-1", "area-1", "pregunta-1");
    });
    ...
    suite("create()", function () {
        test("Debe delegar la llamada a _create con el objeto recibido", function () {
            ...
            const pruebaClaveAreaPregunta = new PruebaClaveAreaPregunta();
            pruebaClaveAreaPregunta.orden = 1;

            const resultado = cut.create(pruebaClaveAreaPregunta);

            chai.assert.equal(resultado, mockPromise);
            chai.assert.isTrue(stubFetch.calledOnce);
            chai.assert.isTrue(stubFetch.calledWithExactly(pruebaClaveAreaPregunta));
        });
    });
    ...
    suite("update()", function () {
        test("Debe delegar la llamada a _update con los parámetros recibidos", function () {
            ...
            const idParam = "123";
            const pruebaClaveAreaPregunta = new PruebaClaveAreaPregunta();
            pruebaClaveAreaPregunta.orden = 2;

            const resultado = cut.update(idParam, pruebaClaveAreaPregunta);

            chai.assert.equal(resultado, mockPromise);
            chai.assert.isTrue(stubFetch.calledOnce);
            chai.assert.isTrue(stubFetch.calledWithExactly(idParam, pruebaClaveAreaPregunta));
        });
    });

...
describe("PreguntaAreaDAO Integration Tests", function () {
  describe("Pruebas con servidor en línea", function () {
    ...
    before(function (done) {
      ...
      preguntaDAO.create(preguntaPadre)
        .then((responseP) => {
    ...
    after(function (done) {
      ...
      if (idPreguntaAreaCreado) {
        promesasLimpieza.push(
          cut.delete(idPreguntaAreaCreado).catch(() => console.warn("PreguntaArea ya estaba eliminado"))
        );
      }
      ...
      Promise.all(promesasLimpieza)
        .then(() => {
          ...
          if (idPreguntaCreado) {
            promesasPadres.push(preguntaDAO.delete(idPreguntaCreado).catch(() => true));
          }

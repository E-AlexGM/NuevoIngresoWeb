...
suite("DefaultDAO Unit Test", function () {
  ...
  suite("_findRange()", function () {
    test("Debe retornar DefaultResponse con datos y total_datos (200)", function (done) {
      ...
      cut._findRange(0, 10).then((response) => {
        ...
        chai.assert.isTrue(
          stubFetch.calledWithMatch(sinon.match(/first=0&max=10/), { method: "GET" })
        );
        done();
      }).catch(done);
    });
    ...
    test("Debe rechazar con DefaultError cuando el estatus no es 200 (ej. 422)", function (done) {
      ...
      cut._findRange(-1, 100).then(() => done(new Error("Se esperaba rechazo")))
      .catch((e) => {
    ...
    test("Debe rechazar cuando falla el parseo JSON", function (done) {
      ...
      cut._findRange(0, 10).then(() => done(new Error("Se esperaba rechazo")))
      .catch((e) => {
        chai.assert.instanceOf(e, DefaultError);
  ...
  suite("_findById()", function () {
    test("Debe retornar DefaultResponse con los datos de la entidad (200)", function (done) {
      ...
      const mockResponse = new Response(JSON.stringify(mockEntity), { status: 200 });
      stubFetch = sinon.stub(window, "fetch").resolves(mockResponse);

      cut._findById(1).then((response) => {
        chai.assert.instanceOf(response, DefaultResponse);
        chai.assert.deepEqual(response.datos, mockEntity);
        chai.assert.isTrue(stubFetch.calledWithMatch(sinon.match(/1$/), { method: "GET" }));
        done();
      }).catch(done);
    });
    ...
    test("Debe rechazar con DefaultError cuando el estatus no es 200", function (done) {
      ...
      cut._findById(99).then(() => done(new Error("Se esperaba rechazo")))
      .catch((e) => {
        chai.assert.instanceOf(e, DefaultError);
    ...
    test("Debe rechazar cuando falla el parseo JSON", function (done) {
      ...
      cut._findById(1).then(() => done(new Error("Se esperaba rechazo")))
      .catch((e) => {
        chai.assert.instanceOf(e, DefaultError);
        chai.assert.include(e.mensaje, "Error al parsear los datos");
        done();
  ...
  suite("_create()", function () {
    test("Debe retornar DefaultResponse con el ID y location (201)", function (done) {
      ...
      cut._create({ nombre: "Nuevo Item" }).then((response) => {
        chai.assert.instanceOf(response, DefaultResponse);
        chai.assert.equal(response.datos.id, "5");
        chai.assert.equal(response.datos.location, "http://localhost:9080/admision-api/v1/area/5");
        chai.assert.isTrue(stubFetch.calledWithMatch(sinon.match.any, { method: "POST" }));
        done();
      }).catch(done);
    });
    ...
    test("Debe rechazar leyendo el header Process-Error cuando falla (400)", function (done) {
      ...
      cut._create({}).then(() => done(new Error("Se esperaba rechazo")))
      .catch((error) => {
        chai.assert.instanceOf(error, DefaultError);
        chai.assert.equal(error.mensaje, "Error de validacion");
        done();
      });
    });
    ...
    test("Debe rechazar leyendo el header Wrong-Parameter cuando falla", function (done) {
      ...
      .catch((error) => {
        chai.assert.instanceOf(error, DefaultError);
        chai.assert.equal(error.mensaje, "Parametro invalido");
        done();
      });
    });
    ...
    test("Debe rechazar con mensaje generico cuando no vienen headers de error", function (done) {
      ...
      .catch((error) => {
        chai.assert.instanceOf(error, DefaultError);
        chai.assert.equal(error.mensaje, "Error del servidor: 500");
        done();
      });
    });
  ...
  suite("_update()", function () {
    test("Debe retornar DefaultResponse actualizado (200)", function (done) {
      ...
      cut._update(1, mockEntity).then((response) => {
        chai.assert.instanceOf(response, DefaultResponse);
        chai.assert.deepEqual(response.datos, mockEntity);
        chai.assert.isTrue(stubFetch.calledWithMatch(sinon.match(/1$/), { method: "PUT" }));
        done();
      }).catch(done);
    });
    ...
    test("Debe rechazar con DefaultError cuando el estatus no es 200", function (done) {
      ...
      cut._update(1, {}).then(() => done(new Error("Se esperaba rechazo")))
      .catch((e) => {
        chai.assert.instanceOf(e, DefaultError);
  ...
  suite("_delete()", function () {
    test("Debe retornar DefaultResponse con datos en null (204)", function (done) {
      ...
      cut._delete(1).then((response) => {
        chai.assert.instanceOf(response, DefaultResponse);
        chai.assert.isNull(response.datos);
        chai.assert.isTrue(stubFetch.calledWithMatch(sinon.match(/1$/), { method: "DELETE" }));
        done();
      }).catch(done);
    });
    ...
    test("Debe rechazar con DefaultError cuando el estatus no es 204", function (done) {
      ...
      cut._delete(1).then(() => done(new Error("Se esperaba rechazo")))
      .catch((e) => {
        chai.assert.instanceOf(e, DefaultError);
  ...
  suite("Métodos Públicos (Throw Error)", function () {
    test("Deben lanzar error por no estar implementados", function () {
      chai.assert.throws(() => cut.findRange(0, 10), Error, "Metodo no implementado");
      chai.assert.throws(() => cut.findById(1), Error, "Metodo no implementado");
      chai.assert.throws(() => cut.create({}), Error, "Metodo no implementado");
      chai.assert.throws(() => cut.update(1, {}), Error, "Metodo no implementado");

...
suite("TipoPruebaDAO Unit Test", function () {
    var cut;
    var stubFetch;
    ...
    suite("create()", function () {
        test("Debe delegar la llamada a _create con el objeto recibido", function () {
            ...
            cut.create(tipoPrueba);
            chai.assert.isTrue(stubFetch.calledOnce);
            chai.assert.isTrue(stubFetch.calledWithExactly(tipoPrueba));
        });
    });
    ...
    suite("update()", function () {
        test("Debe delegar la llamada a _update con los parámetros recibidos", function () {
            ...
            cut.update(idParam, tipoPrueba);
            chai.assert.isTrue(stubFetch.calledOnce);
            chai.assert.isTrue(stubFetch.calledWithExactly(idParam, tipoPrueba));
        });
    });

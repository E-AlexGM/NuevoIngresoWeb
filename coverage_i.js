...
// Interceptamos mocha.run
global.mocha.run = function() {
    // Interceptado: silenciado intencionalmente.
};

originalSetup({ ui: 'bdd', reporter: 'spec' });
...
function buscarPruebas(directorio) {
    ...
    for (const elemento of elementos) {
        ...
        } else if (rutaCompleta.endsWith('_it.js')) { 
...
if (archivosAFiltrar.length === 0) {
    console.log(`No se encontraron pruebas de integración que coincidan con el filtro: "${filtroUsuario}"`);
    if (fs.existsSync(ARCHIVO_INYECTOR_TMP)) fs.unlinkSync(ARCHIVO_INYECTOR_TMP);
    process.exit(0);
}

console.log(`Filtro aplicado: "${filtroUsuario || 'Ninguno (Ejecutando todas las pruebas de integración)'}"`);
console.log(`Se encontraron ${archivosAFiltrar.length} archivos de prueba de integración. Empezando ejecucion...\n`);
...
for (const archivo of archivosAFiltrar) {
    ...
    try {
        ...
        console.error(`\nError: Fallo la prueba de integración en el archivo ${archivo}`);
